    ...
    fn parse_atom(&mut self) -> Result<Expression, SyntaxError> {
        match self.next(ProgramContext::NormalContext) {
            ...
            Ok(_) => Err(SyntaxError {
                error_type: SyntaxErrorType::AtomExpected,
                context: self.context.clone(),
            }),
            Err(lex_error) => Err(SyntaxError {
                error_type: SyntaxErrorType::LexerError(lex_error.error_type),
                context: lex_error.context,
            }),
        }
    }
}
